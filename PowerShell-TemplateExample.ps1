...
<#  
.SYNOPSIS  
    A template that is used as the basis of future code.

.DESCRIPTION
    Running this template will test the error logging and windows forms functions. Nothing useful will be performed.

.PARAMETER  
    [string] RandomVariable1  
    A string value that is written to the screen.
    Default value: Random Variable 1

.PARAMETER  
    [string] RandomVariable2 
    A string value that is written to the screen.
    Default value: Random Variable 2
...
.EXAMPLE
    PowerShell-TemplateExample.ps1
    Will run with default values writen to the screen and produces error log output.

.EXAMPLE
    PowerShell-TemplateExample.ps1 Blah Blah
    Will run with values Blah Blah writen to the screen and produces error log output.
...
Param(
    [parameter(Position=1)][string]$RandomVariable1 = "Random Variable 1",
    [parameter(Position=2)][string]$RandomVariable2 = "Random Variable 2",
...
# Create an error for the logs
Write-Error "Test Error"

# Prompt for a text file and output to the screen 
$TextFile = Select-GUI -File -Ext txt | ForEach {Get-Content $_}
Write-Host $TextFile
